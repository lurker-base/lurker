...
# Navigation with become added back
NEW_NAV = '''    <nav class="nav">
        ...
        <a href="story.html" class="nav-link{active_story}">story</a>
        <a href="live.html" class="nav-link{active_live}">live</a>
        <a href="become.html" class="nav-link{active_become}">become</a>
...
def get_active_class(filename):
    mapping = {
        ...
        'story.html': 'active_story',
        'live.html': 'active_live',
        'become.html': 'active_become',
...
def process_file(filepath):
    ...
    new_nav = NEW_NAV
    for key in ['active_lurker', 'active_token', 'active_story', 'active_live', 'active_become', 'active_proof', 'active_predictions', 'active_subscribe']:
        if key == active:
...
def main():
    ...
    print(f"\nUpdated {len(html_files)} files - 'become' added back")

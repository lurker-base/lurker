...
const CONFIG = {
    ...
    // Clanker Factory sur Base
    clankerFactory: '0x28bE1a58BF350F5b1E7A1cEb4a496071Ca8D0E20',
    // Bankr Factory
    bankrFactory: '0x123...', // TODO: trouver l'adresse
...
async function fetchRecentTokens() {
    ...
    // Search strategies for recent tokens
    const searchStrategies = [
        { query: 'clanker', source: 'clanker' },
        { query: 'bankr', source: 'bankr' },
        { query: 'base', source: 'base' },
        { query: 'WETH', source: 'uniswap' },
        { query: 'USDC', source: 'uniswap' }
    ];
    
    for (const strategy of searchStrategies) {
        try {
            const res = await axios.get(
                `https://api.dexscreener.com/latest/dex/search?q=${strategy.query}`,
                { timeout: 10000 }
            );
            if (res.data?.pairs) {
                for (const pair of res.data.pairs) {
                    if (pair.chainId === 'base' && pair.baseToken) {
                        ...
                        // Only tokens < 2 hours old
                        if (age < 2) {
                            const source = detectSource(pair) || strategy.source;
                            if (!tokens.has(pair.baseToken.address)) {
                                tokens.set(pair.baseToken.address, {
            ...
            console.log(`[DEX] Search error for ${strategy.query}:`, e.message);
        }
    }
    
    console.log(`[DEX] Found ${tokens.size} recent tokens (< 2h)`);
...
async function scan() {
    console.log('[LIVE] =========================================');
    console.log('[LIVE] Scanning for NEW tokens (max 60min old)...');
    console.log('[LIVE] =========================================');
...
// Run
console.log('[LIVE] LURKER Scanner v3 — Optimized DexScreener');
console.log('[LIVE] Focus on pairs < 2h old from Clanker/Bankr/Uniswap');
